            ...
            <div class="col-md-12 grid-margin">
              <div class="row">
                <?php
                  ...
                  $data=$mysqli->common_select('bus','*',$where);
                  if(!$data['error']){
                    $data=$data['data'][0];
                  }
                ?>
                  ...
                  <form class="forms-sample" method="post" action="">
                    <div class="form-group">
                      ...
                      <select class="form-control" id="ticket" name="ticket">
                        <?php
                          $bus_company=$mysqli->common_select('ticket');
                          if(!$ticket['error']){
                            foreach($tiket['data'] as $d){
                        ?>
                          <option value="<?= $d->id ?>" <?= $data->ticket==$d->id ? "selected":""?>><?= $d->name ?></option>
                        <?php } } ?>
                      </select>
                    </div>
